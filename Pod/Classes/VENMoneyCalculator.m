...
- (NSString *)evaluateExpression:(NSString *)expressionString {
    ...
    if ([result isKindOfClass:[NSNumber class]]) {
        ...
        if (integerExpression == floatExpression) {
			...
			NSString *noWhiteSpaceMoneyFormattedNumber = [moneyFormattedNumber stringByReplacingOccurrencesOfString:@" " withString:@""];
            return [noWhiteSpaceMoneyFormattedNumber stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
        }
    } else {
        return nil;
    }
