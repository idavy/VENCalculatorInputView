...
- (void)calculatorInputView:(VENCalculatorInputView *)inputView didTapKey:(NSString *)key {
    ...
    if ([key isEqualToString:@"+"] ||
		...
		if ([self.text isEqualToString:[self decimalSeparator]]) {
			return;
		}
...
- (void)calculatorInputViewDidTapDeleteKey:(VENCalculatorInputView *)calculatorInputView
{
	self.text = @"";
}
- (void)calculatorInputViewDidTapConfirmKey:(VENCalculatorInputView *)calculatorInputView
{
	...
	if ([self.tapDelegate respondsToSelector:@selector(calculatorInputViewDidTapConfirmKey:)]) {
		[self.tapDelegate calculatorInputViewDidTapConfirmKey:calculatorInputView];
	}
}
- (void)calculatorInputViewDidTapFunctionKey:(VENCalculatorInputView *)calculatorInputView
{
	if ([self.tapDelegate respondsToSelector:@selector(calculatorInputViewDidTapFunctionKey:)]) {
		[self.tapDelegate calculatorInputViewDidTapFunctionKey:calculatorInputView];
	}
}
